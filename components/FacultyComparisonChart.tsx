...
export default function FacultyComparisonChart({
  ...
  const chartData = useMemo(() => {
    ...
    return stats.map((s) => ({
      name: truncateChartLabel(s.faculty, 18),
      fullName: s.faculty,
      count: s.count,
  ...
  const chartHeight = Math.max(280, Math.min(640, chartData.length * 40 + 72));
      ...
      <ResponsiveContainer width="100%" height={chartHeight}>
        ...
        >
          ...
          <XAxis
            ...
            label={{
              value: isActivity ? "جمع تعداد" : "میانگین امتیاز",
